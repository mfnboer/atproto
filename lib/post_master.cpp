...
void PostMaster::addQuoteToPost(AppBskyFeed::Record::Post& post, const QString& quoteUri, const QString& quoteCid)
{
    ...
    post.mEmbed = std::make_shared<AppBskyEmbed::Record>();
...
void PostMaster::addGalleryImageToPost(AppBskyFeed::Record::Post& post, Blob::SharedPtr blob, int width, int height, const QString& altText)
{
    if (!post.mEmbed)
    {
    ...
    else if (std::holds_alternative<AppBskyEmbed::Record::SharedPtr>(*post.mEmbed))
    {
        ...
        recordWithMedia->mRecord = std::make_shared<AppBskyEmbed::Record>();
        recordWithMedia->mRecord->mRecord = std::move(ref);
        recordWithMedia->mMedia = std::make_shared<AppBskyEmbed::Gallery>();
    }

    auto image = std::make_shared<AppBskyEmbed::GalleryImage>();
    image->mImage = std::move(blob);
    image->mAlt = altText;
    image->mAspectRatio = std::make_shared<AppBskyEmbed::AspectRatio>();
    image->mAspectRatio->mWidth = width;
    ...
    Q_ASSERT(gallery);
    gallery->mItems.push_back(std::move(image));
}
                                   ...
                                   const ComATProtoRepo::StrongRef::List& associatedRefs)
{
    ...
    if (!post.mEmbed)
    {
        post.mEmbed = std::make_shared<AppBskyEmbed::External>();
        embed = std::get<AppBskyEmbed::External::SharedPtr>(*post.mEmbed).get();
    }
    else
    {
...
void PostMaster::addVideoToPost(AppBskyFeed::Record::Post& post, Blob::SharedPtr blob, int width, int height, const QString& altText, bool isGif)
{
    ...
    if (!post.mEmbed)
    {
        post.mEmbed = std::make_shared<AppBskyEmbed::Video>();
        embed = std::get<AppBskyEmbed::Video::SharedPtr>(*post.mEmbed).get();
    }
    else
    {
        Q_ASSERT(std::holds_alternative<AppBskyEmbed::Record::SharedPtr>(*post.mEmbed));
...
{
    sendInteraction(postUri, feedUri, feedDid, feedContext,
                    AppBskyFeed::Interaction::EventType::RequestMore,
                    successCb, errorCb);
}

void PostMaster::sendInteractionShowLessLikeThis(const QString& postUri, const std::optional<QString>& feedUri, const QString& feedDid, const QString& feedContext,
                                                 const SuccessCb& successCb, const ErrorCb& errorCb)
{
    sendInteraction(postUri, feedUri, feedDid, feedContext,
...
void PostMaster::sendInteraction(const QString& postUri, const std::optional<QString>& feedUri, const QString& feedDid, const QString& feedContext,
                     AppBskyFeed::Interaction::EventType event, const SuccessCb& successCb, const ErrorCb& errorCb)
{
    qDebug() << "Send interaction, postUri:" << postUri << "feedUri:" << feedUri.value_or("") << "feedDid:" << feedDid << "event:" << AppBskyFeed::Interaction::eventTypeToString(event);
    ...
    mClient.sendInteractions(feedUri, {interaction}, feedDid,
        [successCb]{
            if (successCb)
                    successCb();
        },
